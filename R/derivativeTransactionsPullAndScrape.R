#' @title derivativeTransactionsPullAndScrape
#'
...
#' the derivative transactions that meet the keyword crtiera from the
...
#' each row represents a derivative transaction. The transaction
#' observations can be grouped by form through the URL variable within the
#' dataframe.
#' @examples
#' \donttest{
#' dat <- derivativeTransactionsPullAndScrape(quarter = 3, year = 2012,
...
#' @export
derivativeTransactionsPullAndScrape <- function(quarter, year, form, name, email, transactionType = NA, footnoteKeywords = NA, issuerKeywords = NA, issuerTradingSymbol = NA, rptOwnerKeywords = NA) {
  ...
  dat <- as.data.frame(matrix(, ncol=61, nrow=nrow(pull)*6))
  ...
  for (i in pull[,3]) {
    ...
    # These following lines clean the document of uncessary characters and limits us to just the derivitive section
