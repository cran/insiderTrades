#' @title nonderivativeTransactionsScrape
#'
...
#' the nonderivative transactions that meet the keyword crtiera. If no criteria
...
#' @examples
#' \donttest{
#' dat <- nonderivativeTransactionsScrape(index = tempIndex, form = 4,
...
#' @export
nonderivativeTransactionsScrape  <- function(index, form, name, email, transactionType = NA, footnoteKeywords = NA, issuerKeywords = NA, issuerTradingSymbol = NA, rptOwnerKeywords = NA) {
  ...
  dat <- as.data.frame(matrix(, ncol=56, nrow=nrow(index)*6))
