#' @title nonderivativeTransactionsPullAndScrape
#'
...
#' @export
nonderivativeTransactionsPullAndScrape <- function(quarter, year, form, name, email, transactionType = NA, footnoteKeywords = NA, issuerKeywords = NA, issuerTradingSymbol = NA, rptOwnerKeywords = NA) {
  ...
  footnoteScrape <- function(filing, counter, dat, n, k, footnoteNumber, columnNumber) {
    transactionSection <- filing %>% stringr::str_extract(pattern = "<nonDerivativeTransaction>.*?(</nonDerivativeTransaction>)")
  ...
  # SEC Data pull
  pull <- secUrlDownload(quarter = quarter, year = year, form = form, name = name, email = email)
  ...
  dat <- as.data.frame(matrix(, ncol=56, nrow=nrow(pull)*6))
  ...
  counter <- 0
  statusCounter <- 0
  totalRecords <- nrow(pull) # minimize computation
  ...
  for (i in pull[,3]) {
    ...
    keyCount <- formFilterNonderivativeTransactions(filing, footnoteKeywords, issuerKeywords, issuerTradingSymbol, rptOwnerKeywords, transactionType)
    ...
    if (keyCount > 0 | (any(is.na(footnoteKeywords)) & any(is.na(issuerKeywords)) & any(is.na(issuerTradingSymbol)) & any(is.na(rptOwnerKeywords)) & any(is.na(transactionType)))) {
      ...
      filingCount <- stringr::str_count(filing, "nonDerivativeTransaction> <securityTitle>")
      ...
      if (filingCount == 1 & peopleCount == 1) {
        ...
        keyTransactionCount <- transactionFilterNonderivative(filing, footnoteKeywords, issuerKeywords, issuerTradingSymbol, rptOwnerKeywords, transactionType)
        ...
        if (keyTransactionCount > 0 | (any(is.na(footnoteKeywords)) & any(is.na(issuerKeywords)) & any(is.na(issuerTradingSymbol)) & any(is.na(rptOwnerKeywords)) & any(is.na(transactionType)))) {
          ...
          # This section scrapes the information for the columns associated with transaction details. Column index 55 and 56 remain NA since we are able to scrape these transactions
          dat <- transactionScrape(filing = filing, counter = counter, dat = dat, n = 1, k = 23, columnNumber = 1)
          ...
          dat <- footnoteScrape(filing = filing, counter = counter, dat = dat, n = 24, k = 53, footnoteNumber = 1, columnNumber = 24)
        ...
        transactionCount <- stringr::str_count(filing, "nonDerivativeTransaction> <securityTitle>")
        ...
        while (transactionCount >= 1){
          ...
          filing <- filing %>% stringr::str_replace(pattern = "<nonDerivativeTransaction>.*?(</nonDerivativeTransaction>)", replacement = "")
          transactionCount <- stringr::str_count(filing, "nonDerivativeTransaction> <securityTitle>")
        ...
        while (peopleCountTwo >= 1) {
          ...
          if (keyTransactionCount > 0 | (any(is.na(footnoteKeywords)) & any(is.na(issuerKeywords)) & any(is.na(issuerTradingSymbol)) & any(is.na(rptOwnerKeywords)) & any(is.na(transactionType)))) {
            ...
            # This section scrapes the information for the columns associated with transaction details. Column index 55 remains NA since we are able to scrape these transactions
            dat <- transactionScrape(filing = filing, counter = counter, dat = dat, n = 1, k = 23, columnNumber = 1)
            ...
            for (columnName in columnNames[24:53]) {
            ...
            filingFootnotes <- filing %>% stringr::str_extract(pattern = "<nonDerivativeTransaction>.*?(</nonDerivativeTransaction>)")
            ...
            dat[counter, 56] <- "The transaction values in this observation is an aggregate amount that is shared by the other observations that share the same URL"
          }
        ...
        while (transactionCount >= 1){
          ...
          if (keyTransactionCount > 0 | (any(is.na(footnoteKeywords)) & any(is.na(issuerKeywords)) & any(is.na(issuerTradingSymbol)) & any(is.na(rptOwnerKeywords)) & any(is.na(transactionType)))) {
            ...
            dat <- transactionScrape(filing = filing, counter = counter, dat = dat, n = 13, k = 23, columnNumber = 13)
            ...
            dat[counter, 56] <- "This transaction may not be a valid to key word conditions based upon the structure of many reporting owners. This transaction must be checked by hand."
  ...
  dat2[,4:23] <- lapply(dat2[,4:23], function(x) { as.character(gsub('<footnoteId id="F([1-9]|[1-9][0-9])"/>', "", x))})
