#' @title nonderivativeHoldingsPullAndScrape
#'
...
#' the nonderivative holdings that meet the keyword crtiera from the
#' specified financial quarter. If no criteria is set, all holdings from
...
#' @export
nonderivativeHoldingsPullAndScrape <- function(quarter, year, form, name, email, footnoteKeywords = NA, issuerKeywords = NA, issuerTradingSymbol = NA, rptOwnerKeywords = NA) {
  ...
  dat <- as.data.frame(matrix(, ncol=49, nrow=nrow(pull)*6))
